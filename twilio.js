...
export async function sendOTP(phoneNumber) {
  try {
    ...
    if (TESTING_MODE) {
      // In testing mode, generate and store OTP locally
...
// Check if OTP has expired (45 seconds)
export function isOTPExpired() {
  ...
  return diff > 45 * 1000; // 45 seconds in milliseconds
}
...
export function getOTPRemainingTime() {
  ...
  const now = Date.now();
  const diff = now - parseInt(sentAt);
  const remaining = 45 - Math.floor(diff / 1000); // 45 seconds
